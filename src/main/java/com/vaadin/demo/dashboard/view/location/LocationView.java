package com.vaadin.demo.dashboard.view.location;
...
public final class LocationView extends VerticalLayout implements View {
    ...
    private BeanItemContainer<Location> beansContainer = null;
    ...
    private Component buildToolbar() {
        ...
        Label title = new Label("Location Requests");
    ...
    private Component buildFilter() {
        ...
        final TextField txtmsisdn = new TextField("MSISDN");
        toolbar.addComponent(txtmsisdn);
        ...
        btnFilter.addClickListener(new ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				...
				} else if(txtmsisdn.getValue().trim().length() <= 0) {
					btnFilter.setComponentError(new UserError("Please provide atleast  MSISDN or Start and End Dates"));
					return;
				}
				
				beansContainer.removeAllItems();
				Collection<Location> location = DashboardUI.getDataProvider()
						.getLocation(txtmsisdn.getValue().trim(),txtStartDate.getValue(), txtEndDate.getValue());
				if(location != null) {
					beansContainer.addAll(location);
				}
			}
		});
    ...
    private Table buildTable() {
        ...
        beansContainer = new BeanItemContainer<Location>(Location.class);
        table.setContainerDataSource(beansContainer);
        
        table.setSortContainerPropertyId("id");
        table.setSortAscending(false);

        table.setVisibleColumns("id", "msisdn", "serviceId", "dateTime", "cellId");
        table.setColumnHeaders("S.no", "MSISDN", "Service Name", "Date/Time", "ATI Response (Cell-ID)");
